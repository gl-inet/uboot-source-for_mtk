...
const static struct gpio_reg_offset_s {
	...
	unsigned short clear_offset;
	unsigned short redge_offset;
	unsigned short fedge_offset;
	unsigned short int_high_offset;
	unsigned short int_low_offset;
	unsigned short int_stat_offset;
	unsigned short edge_stat_offset;
} s_gpio_reg_offset[] = {
	{  0, 31,  0x0, 0x10, 0x20, 030, 0x40, 0x50, 0x60, 0x70, 0x80, 0x90, 0xa0 },
	{ 32, 63, 0x4, 0x14, 0x24, 0x34, 0x44, 0x54, 0x64, 0x74, 0x84, 0x94, 0xa4 },
	{ 64, 95, 0x8, 0x18, 0x28, 0x38, 0x48, 0x58, 0x68, 0x78, 0x88, 0x98, 0xa8 },
};
 ...
 */
unsigned int mtk7628_get_gpio_reg_addr(unsigned short gpio_nr, enum gpio_reg_id id)
{
	int ret = 0;
	const struct gpio_reg_offset_s *p;
	...
	case GPIO_FEDGE:
		...
		break;
	case GPIO_INT_H:
		ret = p->int_high_offset;
		break;
	case GPIO_INT_L:
		ret = p->int_low_offset;
		break;
	case GPIO_INT_STAT:
		ret = p->int_stat_offset;
		break;
	case GPIO_EDGE_STAT:
		ret = p->edge_stat_offset;
	default:
		return 0;
	}
	ret += PRGIO_ADDR;
 ...
 * 	-1:	Invalid parameter
 */
int mtk7628_set_gpio_dir(unsigned short gpio_nr, unsigned short gpio_dir)
{
	int shift;
	...
	reg = mtk7628_get_gpio_reg_addr(gpio_nr, GPIO_DIR);
 ...
 * 	-1:	Invalid parameter
 */
int mtk7628_get_gpio_pin(unsigned short gpio_nr)
{
	int shift;
	unsigned int reg;
	unsigned long val = 0;
	...
	reg = mtk7628_get_gpio_reg_addr(gpio_nr, GPIO_DATA);
	shift = get_gpio_reg_bit_shift(gpio_nr);
 ...
 * 	-1:	Invalid parameter
 */
int mtk7628_set_gpio_pin(unsigned short gpio_nr, unsigned int val)
{
	int shift;
	unsigned int reg;
...
void led_init( void ){
	...
	for ( i = 0; i < ARRAY_SIZE( led ); ++i ) {
		mtk7628_set_gpio_dir( led[i], 0 );	/* Set LED as output */
		mtk7628_set_gpio_pin( led[i], 0 );	/* turn on LED */
	}
}
...
void gpio_init(void)
{
	printf( "MT7628 Ai-BR100 gpio init : WPS / RESET pin\n" );

	mtk7628_set_gpio_dir(WPS_BTN, 1);
}
...
unsigned long DETECT(void)
{
	...
	mtk7628_set_gpio_pin(RST_BTN, 1);

	if(!mtk7628_get_gpio_pin(RST_BTN)) {
		key = 1;
...
unsigned long DETECT_WPS(void)
{
	...
	if(!mtk7628_get_gpio_pin(WPS_BTN)) {
		key = 1;
...
void LEDON( void )
{
	mtk7628_set_gpio_pin( WAN_LED, 0 );
	mtk7628_set_gpio_pin( WIFI_2G_LED, 0 );
}

void LEDOFF( void )
{
	mtk7628_set_gpio_pin( WAN_LED, 1 );
	mtk7628_set_gpio_pin( WIFI_2G_LED, 1 );
}
...
void LED_WLAN_ON( void )
{
	mtk7628_set_gpio_pin( WIFI_2G_LED, 0 );
}

void LED_WLAN_OFF( void )
{
	mtk7628_set_gpio_pin( WIFI_2G_LED, 1 );
}
