...
int do_run(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (!strncmp(argv[1], "lu", 2)) {
		 ...
		 */
		if (run_command("tftpboot 0x80100000 uboot-gl-mt300an.bin", 0)) {
			run_command("erase 0xbc000000 0xbc030000", 0);
			run_command("cp 0x80100000 0xbc000000 0x30000", 0);
		}
		return 0;
	} else if (!strncmp(argv[1], "lf", 2)) {
		if (run_command("tftpboot 0x80100000 openwrt-gl-mt300an.bin", 0)) {
			/* erase flash */
			sprintf(cmd, "erase 0xbc050000 +%x", NetBootFileXferSize + 0x10000);
			run_command(cmd, 0);

			/* burn to flash */
			memset(cmd, 0x0, 128);
			sprintf(cmd, "cp 0x80100000 0xbc050000 %x", NetBootFileXferSize + 0x10000);
			run_command(cmd, 0);
		}
		return 0;
	} else if (!strncmp(argv[1], "lc", 2)) {
		cp_flash_to_ram(0xbc040000, 0x80040000, 0x4000);
		if (run_command("tftpboot 0x80044000 config.bin", 0)) {
			copy_modify_mac();
			copy_mac();
...
U_BOOT_CMD(
	...
	"run   - tftp-erase-cp\n",
	"\nrun lu:\n"
	"\t - download uboot-gl-mt300an.bin from your tftp server and replace the current one\n"
	"run lf:\n"
	"\t - download openwrt-gl-mt300an.bin from your tftp server and write on flash\n"
	"run lc:\n"
	"\t - download config.bin from your tftp server and change the mac address\n"
);
